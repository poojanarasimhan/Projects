...
<c:if test="${(user.name != null) && (user.type == 'Pharmacy')}">
<div class="jumbotron"><h2 class="text-center">Hospital Management Systems</h2></div>
	...
	<form action="deliverMed.htm" method="get">
	<c:forEach var="pres" items="${prescription}">
	<div class="row">
		<div class="col-md-3">
		<input type="radio" name="selectPres" value="${pres.presID}"/> 	
		</div>
		<div class="col-md-3">
		<input type="text" name="id" value="${pres.presID}" readonly/>
		</div>
		<div class="col-md-3">
		<input type="text" name="id" value="${pres.doctor}" readonly/>
		</div>
		<div class="col-md-3">
		<input type="text" name="id" value="${pres.patient}" readonly/>
		</div>
	</div>
	</c:forEach>
		<input type="submit" value="DELIVER MEDICINE" />
	</form>
	</div>
	<script>
		function formSubmit() {
